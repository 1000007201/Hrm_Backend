...
// getHolidayDateKeys in src/lib/holidays.ts, which is the only intended way
// to build that set.
//
...
// set-building side (src/lib/holidays.ts) formats keys identically; a
...
// Exported so attendance's deriveDailyStatus (src/lib/attendance.ts) reuses
